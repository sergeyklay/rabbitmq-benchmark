...
int rmq_auth(amqp_connection_state_t *conn, rabbit_instance *rmq) {
	...
	switch (reply.reply_type) {
		case AMQP_RESPONSE_NONE:
			OUT_ERROR("No Response from RPC");
			break;
		case AMQP_RESPONSE_LIBRARY_EXCEPTION:
			OUT_ERROR("Possible SSL/TLS error during certificate negotiations.");
			break;
		case AMQP_RESPONSE_SERVER_EXCEPTION:
			OUT_ERROR("Issue could be authentication or issue with the MQ Channel.");
			switch (reply.reply.id) {
				...
				default:
					OUT_ERROR("Unknown server error, method id 0x%08X\n", reply.reply.id);
					break;
			}
		default:
			OUT_ERROR("Unknown error, method id 0x%08X\n", reply.reply.id);
			break;
	}
